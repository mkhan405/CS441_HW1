...
//      val computedEmbeddings = model.getLayer(0).getParam("W")
//
...
//            val tokenized_word = word.split(" ").flatMap(w => utils.encoding.encode(w).toArray)
//            log.debug(tokenized_word.mkString(" "))
//            try {
//              // Get Embedding Prediction
//              val embedding = Nd4j.createFromArray(
//                tokenized_word.map(idx => computedEmbeddings.getRow(idx).toDoubleVector).toArray
//              )
//              // Send to reducer
//              context.write(new Text(word), new VectorWritable(embedding.toFloatVector))
//            } catch {
//              case e: Exception =>
